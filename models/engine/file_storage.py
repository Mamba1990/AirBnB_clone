#!/usr/bin/python3
"""Defines the FileStorage class."""
import json
from models.base_model import BaseModel
from models.user import User
from models.state import State
from models.city import City
from models.place import Place
from models.amenity import Amenity
from models.review import Review


class FileStorage:
    """Defines a storage abstaction of an engine.

    Attributes:
        __file_path (str): The file's name to save objects to.
        __objects (dict): The instantiated objects' dictionary.
    """
    __file_path = "file.json"
    __objects = {}

    def all(self):
        """Displays the dictionary __objects."""
        return FileStorage.__objects

    def new(self, obj):
        """Setting  in __objects obj with key <obj_class_name>.id"""
        oc_name = obj.__class__.__name__
        FileStorage.__objects["{}.{}".format(oc_name, obj.id)] = obj

    def save(self):
        """Serializing __objects to the JSON file __file_path."""
        o_dict = FileStorage.__objects
        obj_dict = {obj: o_dict[obj].to_dict() for obj in o_dict.keys()}
        with open(FileStorage.__file_path, "w") as fi:
            json.dump(obj_dict, fi)

    def reload(self):
        """Deserializing the JSON file, if it exists."""
        try:
            with open(FileStorage.__file_path) as fi:
                obj_dict = json.load(fi)
                for ob in obj_dict.values():
                    clsName = ob["__class__"]
                    del ob["__class__"]
                    self.new(eval(clsName)(**ob))
        except FileNotFoundError:
            return
